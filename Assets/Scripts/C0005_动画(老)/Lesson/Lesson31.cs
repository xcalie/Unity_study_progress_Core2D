...
public class Lesson31 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            animation.CrossFadeQueued("3");
        }

        //浅入播放
        if (Input.GetKeyDown(KeyCode.F))
        {
            animation.CrossFade("4");
        }

        //判断是否在播放某个动画
        if (Input.GetKeyDown(KeyCode.G))
        {
            print(animation.IsPlaying("4"));
        }
        ...
        animation["4"].layer = 1;
        //设置权重
        animation["4"].weight = 0.5f;
        //混合模式 叠加还是混合
        animation["4"].blendMode = AnimationBlendMode.Additive;
        //设置混组相关骨骼信息
        animation["4"].AddMixingTransform(this.transform);
